...
nc_size,num_antibody = 50,[100,162,220]
...
if (bias_mode == 'H'):
	...
	dirname1='Zbias_N-'+str(N)+"_bl-"+str(init_bond_length)
	lambda_zero=1.0
	...
	for blen_scale_factor in blen_fac:
		...
		for nab in num_antibody:
			...
			for ens in range(start_ens,end_ens):
				...
				fp=open('runscript','w')
				fp.write("#!/bin/bash \n")
				runcmd="mpirun -np " + str(n_processor) +" ./"+str(run_exe) 
				fp.write(runcmd)
				fp.close()
				p=subprocess.Popen(["bash","runscript"])
				os.chdir(subdir)
			p.wait()
			os.chdir(curdir)
		os.chdir(curdir_out)
