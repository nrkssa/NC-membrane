...
bias_mode ='N'
window_conf_interval=num_moves/50
...
codehome=os.getcwd()
exec_name='free_ener_32'

if (bias_mode == 'H'):
	dirname1='Hbias_N-'+str(N)+"_bl-"+str(init_bond_length)
	lambda_zero=1.0

elif (bias_mode == 'Z'):
	dirname1='Rbias_N-'+str(N)+"_bl-"+str(init_bond_length)
	lambda_zero=1.0

elif (bias_mode == 'T'):
	dirname1='TI_N-'+str(N)+"_bl-"+str(init_bond_length)
	lambda_zero=0.0

else:
	dirname1='Equm_N'+str(N)+"_bl-"+str(init_bond_length)
	lambda_zero=1.0
...
for z_start_min in zvalues:

	bias_pot_fluctz,z_start,zstart_window,numz_windows,zwindow_step_size=0.0,z_start_min,0,1,0.2
	bias_pot_fluctH,H_start,Hstart_window,numH_windows,Hwindow_step_size=0.0,0.0,0,1,0.0015
	...
	for blen_scale_factor in blen_fac:
		...
		for nab in num_antibody:
			...
			for ens in range(start_ens,end_ens):
				run_exe='M-'+str(nab)+'-'+str(kappa)+'-'+str(blen_scale_factor)+'-'+str(ens)
...
#					memb_res_file=restart_dir+'/membrane_state-'+str(nfiles)+'.dump'
				...
				if bias_mode == 'Z':
					...
					Href,Hstart,Hend = H_start,0.0,1
					zref,zstart,zend = z_start,0,1
				elif bias_mode == 'T':
					Href,Hstart,Hend = 0.0,Hstart_window,numH_windows
				...
				['osd_print_interval',osd_print_interval],
				['lambda',lambda_zero]]                    #options are advance or recede
				...
				fp.write('#PBS -l nodes=1:ppn='+str(n_processor)+'\n')
				...
				fp.write('module load openmpi-1.6.5-gcc\n')
