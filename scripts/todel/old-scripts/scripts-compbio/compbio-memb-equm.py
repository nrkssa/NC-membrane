...
if (bias_mode == 'H'):
	dirname1='Memb_Hbias_N-'+str(N)+"_bl-"+str(init_bond_length)
	dirname1_rs='Memb_Hbias_N-'+str(N)+"_bl-"+str(init_bond_length)+'-rs'
elif (bias_mode == 'Z'):
	dirname1='Memb_Zbias_N-'+str(N)+"_bl-"+str(init_bond_length)
	dirname1_rs='Memb_Zbias_N-'+str(N)+"_bl-"+str(init_bond_length)+'-rs'
else:
	dirname1='Memb-N'+str(N)+"_bl-"+str(init_bond_length)
	dirname1_rs='Memb-N'+str(N)+"_bl-"+str(init_bond_length)+'-rs'
...
for blen_scale_factor in blen_fac:
	...
	dirname20='kappa-'+str(kappa)+'-pressure-'+str(pressure)+'-czero-'+str(czero)+'-conc-'+str(czero_conc)
	subprocess.call(["mkdir","-pv",dirname20])
	os.chdir(dirname20)
	subdir = os.getcwd()
	...
	for ens in range(start_ens,end_ens):
		run_exe='memb-equm-kappa'+str(kappa)+'-ens-'+str(ens)
		dirname30='ENS-'+str(ens)
		os.mkdir(dirname30)
		os.chdir(dirname30)
		subdir1=os.getcwd()
		os.mkdir('RUNDIR')
		...
		if restart_memb ==1:
			restart_dir=codehome+'/'+dirname1_rs+'/'+dirname10+'/'+dirname20+'/'+dirname30+'/SYS_STATE/DUMP'
			print restart_dir
