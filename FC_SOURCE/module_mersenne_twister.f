...
!   The Fortran 90 interfaces random_seed_mt and random_number_mt are overloaded
...
!   The full generator state can be set via the standard interface random_seed_mt,
!   but it is recommended to use this method only to restore saved states, e.g.
!     call random_seed_mt(size=lsave)  ! get size of generator state seed array
!     allocate isave(lsave)         ! allocate seed array
!     call random_seed_mt(get=isave)   ! fill seed array (then maybe save to disk)
!     call random_seed_mt(put=isave)   ! restore state (after read from disk maybe)
!   Locally kept generator states can also be saved in a seed array, e.g.
!     type(random_stat):: stat
!     call random_seed_mt(get=isave,stat=stat)  ! fill seed array
!     call random_seed_mt(put=isave,stat=stat)  ! restore state
...
!     enddo
!     do i=1,8                               ! threadable loop
!       call random_number_mt(harvest,stat(i))  ! thread-safe call
!     enddo
!     do i=1,8                               ! threadable loop
...
!   
! Public Subprograms:
!   random_seed_mt         determine size or put or get state
...
!     inseed            integer seed input
!   random_number_mt       get mersenne twister random numbers (thread-safe mode)
!     harvest           real(:) numbers output
!     stat              type(random_stat) input
!   random_number_mt       get mersenne twister random numbers (saved mode)
!     harvest           real(:) numbers output
!   random_number_mt       get mersenne twister random numbers (interactive mode)
!     harvest           real(:) numbers output
!     inseed            integer seed input
...
!=============================================================================================================================
!--> 06/12/2013 - modified for use with freeform codes by Ramakrishnan Natesan, University of Pennsylvania
!=============================================================================================================================

module module_mersenne_twister
        private
        public random_stat 													    !  Public declarations
        public random_seed_mt
        public random_setseed
        public random_number_mt
        ...
        integer,parameter:: n=624												    !  Parameters
        ...
        type random_stat													    !  Defined types
          private
          integer:: mti=n+1
          integer:: mt(0:n-1)
          integer:: iset
        ...
        type(random_stat),save:: sstat												    !  Saved data

        interface random_setseed												    !  Overloaded interfaces
          module procedure random_setseed_s
          module procedure random_setseed_t
        end interface
        interface random_number_mt
      ...
      contains
!=============================================================================================================================
!  Subprogram random_seed_mt
!  Sets and gets state; overloads Fortran 90 standard.
!=============================================================================================================================
        subroutine random_seed_mt(size,put,get,stat)
          implicit none
...
!     if(present(put).or.present(get)) call errmsg('RANDOM_SEED: more than one option set - some ignored')
            size=nrest
          elseif(present(put)) then 												    ! restore from seed array
!     if(present(get)) call errmsg('RANDOM_SEED: more than one option set - some ignored')
            if(present(stat)) then
              stat%mti=put(1)
              stat%mt=put(2:n+1)
              stat%iset=put(n+2)
              stat%gset=transfer(put(n+3:nrest),stat%gset)
              if(stat%mti.lt.0.or.stat%mti.gt.n.or.any(stat%mt.eq.0).or. stat%iset.lt.0.or.stat%iset.gt.1) then
...
!         call errmsg('RANDOM_SEED: invalid seeds put - default seeds used')
              ...
              if(sstat%mti.lt.0.or.sstat%mti.gt.n.or.any(sstat%mt.eq.0) .or. sstat%iset.lt.0.or.sstat%iset.gt.1) then
...
!=============================================================================================================================
      ...
      end module module_mersenne_twister
